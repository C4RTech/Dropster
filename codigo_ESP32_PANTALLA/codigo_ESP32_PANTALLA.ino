...
// Interfaz Local Dropster: muestra valores ambientales, evaporador y consumo eléctrico.
...
// Decidir la acción leyendo el texto actual del label para evitar desincronización local
static void event_handler_btn(lv_event_t * e) {
  ...
  if(code == LV_EVENT_CLICKED) {
    ...
    bool currentlyOn = (txt && strstr(txt, "APAGAR") != NULL); // "APAGAR AWG" => actualmente encendido
    if (currentlyOn) {
      // queremos apagar
      Serial1.println("OFFC"); // Comando para Compresor
...
static void event_handler_vent(lv_event_t * e) {
  ...
  if(code == LV_EVENT_CLICKED) {
    lv_obj_t * label = lv_obj_get_child(obj, 0);
    const char *txt = label ? lv_label_get_text(label) : NULL;
    bool currentlyOn = (txt && strstr(txt, "APAGAR") != NULL); // "APAGAR VENT" => actualmente encendido
    if (currentlyOn) {
      Serial1.println("OFFV");
      lv_label_set_text(label, "ENCEDER VENT");
      lv_obj_set_style_bg_color(obj, COLOR_SECONDARY, 0);
      ventState = false;
    } else {
      Serial1.println("ONV");
      lv_label_set_text(label, "APAGAR VENT");
      lv_obj_set_style_bg_color(obj, COLOR_ACCENT1, 0);
      ventState = true;
    }
  }
...
static void event_handler_pump(lv_event_t * e) {
  ...
  if(code == LV_EVENT_CLICKED) {
    lv_obj_t * label = lv_obj_get_child(obj, 0);
    const char *txt = label ? lv_label_get_text(label) : NULL;
    bool currentlyOn = (txt && strstr(txt, "APAGAR") != NULL); // "APAGAR BOMB" => actualmente encendida
    if (currentlyOn) {
      Serial1.println("OFFB");
...
void lv_create_main_gui(void) {
    ...
    lv_obj_align(data_panel, LV_ALIGN_TOP_MID, 0, 385);
    ...
    for (int i = 7; i <= 10; i++, idx++) { // Voltaje, Corriente, Potencia, Energia
        lv_obj_t * cont = lv_obj_create(data_panel);
        lv_obj_set_size(cont, LV_PCT(100), LV_SIZE_CONTENT);
        lv_obj_set_style_bg_opa(cont, LV_OPA_0, 0);
        lv_obj_set_style_border_width(cont, 0, 0);
    ...
    lv_obj_set_size(btn_cont, 220, 140);
    lv_obj_align(btn_cont, LV_ALIGN_BOTTOM_MID, 0, 132);
    ...
    lv_obj_align_to(mode_btn_small, agua_cont, LV_ALIGN_OUT_BOTTOM_MID, 0, 25);

    // Botón Ventilador
    ui_btn_vent = lv_button_create(btn_cont);
    lv_obj_set_size(ui_btn_vent, 180, 36);
    ...
    lv_label_set_text(label_vent, "ENCEDER VENT");
    ...
    lv_obj_set_size(ui_btn_pump, 180, 36);
...
void update_labels(float vals[15]) {
    // Valores ambientales (0..4). Mostrar '--' cuando no hay dato.
    for (int i = 0; i <= 4; i++) {
        if (!isnan(vals[i])) {
    ...
    // Evaporador: Temp <- vals[6], Hum <- vals[7]
    if (!isnan(vals[6])) {
        lv_label_set_text_fmt(labels[5], formats[5], vals[6]);
    } else {
        lv_label_set_text(labels[5], "--");
    }
    if (!isnan(vals[7])) {
        lv_label_set_text_fmt(labels[6], formats[6], vals[7]);
    } else {
        lv_label_set_text(labels[6], "--");
    }

    // Consumo eléctrico: Voltaje/Corriente/Potencia/Energia <- vals[8..11]
    if (!isnan(vals[8])) {
        lv_label_set_text_fmt(labels[7], formats[7], vals[8]);
    } else {
        lv_label_set_text(labels[7], "--");
    }
    if (!isnan(vals[9])) {
        lv_label_set_text_fmt(labels[8], formats[8], vals[9]);
    } else {
        lv_label_set_text(labels[8], "--");
    }
    if (!isnan(vals[10])) {
        lv_label_set_text_fmt(labels[9], formats[9], vals[10]);
    } else {
        lv_label_set_text(labels[9], "--");
    }
    if (!isnan(vals[11])) {
        lv_label_set_text_fmt(labels[10], formats[10], vals[11]);
    } else {
        lv_label_set_text(labels[10], "--");
    }

    // Agua almacenada (vals[5])
    if (!isnan(vals[5])) {
        ...
        if (c == '\n') {
            ...
            // Mensajes de estado o control desde AWG: MODE:, COMP:, VENT:, PUMP:, CTRL:
                ...
                if (ui_btn_vent) {
                    ...
                    if (on) {
                        lv_label_set_text(lbl, "APAGAR VENT");
                        lv_obj_set_style_bg_color(ui_btn_vent, COLOR_ACCENT1, 0);
                    } else {
                        lv_label_set_text(lbl, "ENCEDER VENT");
                        lv_obj_set_style_bg_color(ui_btn_vent, COLOR_SECONDARY, 0);
                    }
                }
            }
            ...
            // Mensaje CSV completo (valores)
                ...
                int fieldCount = idx;
                for (int i = idx; i < 15; i++) vals[i] = NAN;

                // Depuración mínima por USB (no usar Serial1 para no interferir con AWG)
                Serial.print("DBG: fields=");
